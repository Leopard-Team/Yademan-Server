...
try {
    ...
    if (isset($_POST['phoneNumber']) && isset($_POST['PSignUp']) && isset($_POST['USignUp'])){
        ...
        elseif (isUserExisted(NULL, $phone_number, $conn))
            ...
            $sql = "INSERT INTO students (user_name,password, phoneNumber,salt,hash)
                            VALUES('$username','$pass','$phone_number','$salt','$hash')";
            if ($conn->query($sql)) {
                echo true;
 ...
 */
function isUserExisted($username = NULL, $phone_number = NULL, $conn)
{
    $tableName = "students";
    if ($username != NULL) {
